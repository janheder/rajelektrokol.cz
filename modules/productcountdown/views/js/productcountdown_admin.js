...
$(document).ready(function () {
    $('.bootstrap .productcountdown > tbody  tr  td.dragHandle').wrapInner('<div class="positions"/>');
    $('.bootstrap .productcountdown > tbody  tr  td.dragHandle').wrapInner('<div class="dragGroup"/>');
    initAjaxTabs();
    initAutocomplite();
...
function initAjaxTabs() {
    ...
    $('.productcountdown > tbody tr').each(function () {
        var id = $(this).find('td:first').text();
        $(this).attr('id', 'item_' + id.trim());
    });

    var $tabslides = $('.productcountdown > tbody');

    $tabslides.sortable({
        cursor: 'move',
        items: '> tr',
        update: function (event, ui) {
            $('.productcountdown > tbody > tr').each(function (index) {
