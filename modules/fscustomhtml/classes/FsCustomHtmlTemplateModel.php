 ...
 */
class FsCustomHtmlTemplateModel extends ObjectModel
{
    /** @var int */
    public $id;
    ...
    /** @var string */
    public $template_editor = 'codemirror';
    ...
    public static $definition = [
        'table' => 'fsch_template',
        'primary' => 'id_fsch_template',
        'fields' => [
            'title' => ['type' => self::TYPE_STRING, 'validate' => 'isGenericName', 'required' => true],
            'template' => ['type' => self::TYPE_HTML, 'validate' => 'isAnything'],
            'template_editor' => ['type' => self::TYPE_STRING, 'validate' => 'isGenericName'],
    ...
    {
        $array = [
            'id_fsch_template' => $this->id,
            'title' => $this->title,
            'template' => $this->template,
            'template_editor' => ($this->template_editor) ? $this->template_editor : 'codemirror',
        ];
    ...
    {
        $where = [];
        if (isset($filter['id_fsch_template']) && $filter['id_fsch_template'] != '') {
            $where[] = 'c.`id_fsch_template` = \'' . pSQL($filter['id_fsch_template']) . '\'';
        }
    ...
    public static function getCached($id)
    {
        if (!isset(self::$cache[$id])) {
            ...
            self::$cache[$id] = $template->template;
        }
