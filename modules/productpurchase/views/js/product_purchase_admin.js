...
$(document).ready(function () {
    $('.bootstrap .productpurchase > tbody  tr  td.dragHandle').wrapInner('<div class="positions"/>');
    $('.bootstrap .productpurchase > tbody  tr  td.dragHandle').wrapInner('<div class="dragGroup"/>');
    initAjaxTabs();
    initAutocomplite();
});
...
function initAjaxTabs() {
    ...
    $('.productpurchase > tbody tr').each(function () {
        var id = $(this).find('td:first').text();
        $(this).attr('id', 'item_' + id.trim());
    });

    var $tabslides = $('.productpurchase > tbody');

    $tabslides.sortable({
        cursor: 'move',
        items: '> tr',
        update: function (event, ui) {
            $('.productpurchase > tbody > tr').each(function (index) {
...
function initAutocomplite() {
    $('#product_autocomplete_input').autocomplete(file_theme_url + 'controllers/admin/bon_ajax_products_list.php', {
        minChars: 1,
        autoFill: true,
        max: 20,
        matchContains: true,
