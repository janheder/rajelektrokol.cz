...
$(document).ready(function () {
  ...
  $('.bootstrap .bonlookbookfw > tbody  tr  td.dragHandle').wrapInner(
    '<div class="positions"/>'
  );
  $('.bootstrap .bonlookbookfw > tbody  tr  td.dragHandle').wrapInner(
    '<div class="dragGroup"/>'
...
function initAjaxTabs() {
  $('.bonlookbookfw > tbody tr').each(function () {
    var id = $(this).find('td:first').text();
    $(this).attr('id', 'item_' + id.trim());
  });

  var $tabslides = $('.bonlookbookfw > tbody');
  ...
  $tabslides
    .sortable({
      ...
      update: function (event, ui) {
        $('.bonlookbookfw > tbody > tr').each(function (index) {
          $(this)
            .find('.positions')
            .text(index + 1);
        });
...
function initAutocomplite() {
  $('#product_autocomplete_input').autocomplete(file_theme_url + "controllers/admin/AdminAjaxBonlookbookfwProdutsSearch.php", {
    minChars: 1,
    autoFill: true,
    max: 200,
    matchContains: true,
