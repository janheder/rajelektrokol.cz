...
{block name='content'}
    ...
    {if isset($items) && $items}
        <section id="bonlookbookfw" class="bonlookbookfw-separate bonlookbookfw{if $sliderfw_settings['BON_LOOKBOOKFW_PAGE_SLIDER_DISPLAY_CAROUSEL'].value} swiper{/if}{if !($settingsfw['BON_LOOKBOOKFW_CUSTOM_PAGE_TEXT'].value)} bonlookbookfw-without-text{/if}">

            {if $settingsfw['BON_LOOKBOOKFW_TITLE_CUSTOM_PAGE'].value || $settingsfw['BON_LOOKBOOKFW_SUBTITLE_CUSTOM_PAGE'].value}
                <div class="bonlookbookfw_header {$settingsfw['BON_LOOKBOOKFW_TEXT_POSITION'].value|escape:'htmlall':'UTF-8'}">

                    {if $settingsfw['BON_LOOKBOOKFW_TITLE_CUSTOM_PAGE'].value}
                        <div class="bonlookbookfw_header-title">
                            ...
                            <style>
                                #bonlookbookfw .bonlookbookfw_header-title a:hover {
                                    color: {$settingsfw['BON_LOOKBOOKFW_TITLE_HOVER_COLOR'].value|escape:'htmlall':'UTF-8'};
                                }
                            </style>
                        </div>
                    {/if}

                    {if $settingsfw['BON_LOOKBOOKFW_SUBTITLE_CUSTOM_PAGE'].value}
                        <div class="bonlookbookfw_header-subtitle">
                            <p class="h2" style="color: {$settingsfw['BON_LOOKBOOKFW_SUBTITLE_COLOR'].value|escape:'htmlall':'UTF-8'}">{l s='Stay ahead of the fashion trends with our new selection' mod='bonlookbookfw'}</p>
                        </div>
                    {/if}
                </div>
            {/if}
            <div class="bonlookbookfw_wrapper{if $sliderfw_settings['BON_LOOKBOOKFW_PAGE_SLIDER_DISPLAY_CAROUSEL'].value} swiper-wrapper{/if}">

                {foreach from=$items item=item name=item}

                    {if $smarty.foreach.item.iteration <= $settingsfw['BON_LOOKBOOKFW_CUSTOM_PAGE_LIMIT'].value}
                        <div class="bonlookbookfw_item{if $sliderfw_settings['BON_LOOKBOOKFW_PAGE_SLIDER_DISPLAY_CAROUSEL'].value} swiper-slide{/if}">

                            {if isset($item.title) && $item.title && $settingsfw['BON_LOOKBOOKFW_CUSTOM_ELEM_TITLE'].value ||
                                isset($item.description) && $item.description && $settingsfw['BON_LOOKBOOKFW_CUSTOM_PAGE_TEXT'].value}
                                <div class="bonlookbookfw_item-text">
                                    ...
                                    {/if}
                                    {if isset($item.description) && $item.description && $settingsfw['BON_LOOKBOOKFW_CUSTOM_PAGE_TEXT'].value}
                                    ...
                                    {if isset($points) && $points && $item.subitems}
                                        ...
                                        {foreach from=$points item=point name=point}
                                            ...
                                            {if $item.id == $point.id_tab && $point.status}
                                                <div class="bonlookbookfw_point" style="top: {$point.top|escape:'htmlall':'UTF-8' / $item.image_size[1]|escape:'htmlall':'UTF-8' * 100}%; left: {$point.left|escape:'htmlall':'UTF-8' / $item.image_size[0]|escape:'htmlall':'UTF-8' * 100}%">
            ...
            {if $sliderfw_settings['BON_LOOKBOOKFW_SLIDER_NAV'].value && $sliderfw_settings['BON_LOOKBOOKFW_PAGE_SLIDER_DISPLAY_CAROUSEL'].value}
