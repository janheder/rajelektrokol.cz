...
{block name='content'}
    ...
    {if isset($items) && $items}
        <section id="bonlookbook" class="bonlookbook-separate bonlookbook{if $slider_settings['BON_LOOKBOOK_PAGE_SLIDER_DISPLAY_CAROUSEL'].value} swiper{/if}{if !($settingslb['BON_LOOKBOOK_CUSTOM_PAGE_TEXT'].value)} bonlookbook-without-text{/if}">

            {if $settingslb['BON_LOOKBOOK_TITLE_CUSTOM_PAGE'].value || $settingslb['BON_LOOKBOOK_SUBTITLE_CUSTOM_PAGE'].value}
                <div class="bonlookbook_header {$settingslb['BON_LOOKBOOK_TEXT_POSITION'].value|escape:'htmlall':'UTF-8'}">

                    {if $settingslb['BON_LOOKBOOK_TITLE_CUSTOM_PAGE'].value}
                        <div class="bonlookbook_header-title">
                            ...
                            <style>
                                #bonlookbook .bonlookbook_header-title a:hover {
                                    color: {$settingslb['BON_LOOKBOOK_TITLE_HOVER_COLOR'].value|escape:'htmlall':'UTF-8'};
                                }
                            </style>
                        </div>
                    {/if}

                    {if $settingslb['BON_LOOKBOOK_SUBTITLE_CUSTOM_PAGE'].value}
                        <div class="bonlookbook_header-subtitle">
                            <p class="h2" style="color: {$settingslb['BON_LOOKBOOK_SUBTITLE_COLOR'].value|escape:'htmlall':'UTF-8'}">{l s='Stay ahead of the fashion trends with our new selection' mod='bonlookbook'}</p>
                        </div>
                    {/if}
                </div>
            {/if}
            <div class="bonlookbook_wrapper{if $slider_settings['BON_LOOKBOOK_PAGE_SLIDER_DISPLAY_CAROUSEL'].value} swiper-wrapper{/if}">

                {foreach from=$items item=item name=item}

                    {if $smarty.foreach.item.iteration <= $settingslb['BON_LOOKBOOK_CUSTOM_PAGE_LIMIT'].value}
                        <div class="bonlookbook_item{if $slider_settings['BON_LOOKBOOK_PAGE_SLIDER_DISPLAY_CAROUSEL'].value} swiper-slide{/if}">

                            {if isset($item.title) && $item.title && $settingslb['BON_LOOKBOOK_CUSTOM_ELEM_TITLE'].value ||
                                isset($item.description) && $item.description && $settingslb['BON_LOOKBOOK_CUSTOM_PAGE_TEXT'].value}
                                <div class="bonlookbook_item-text">
                                    ...
                                    {/if}
                                    {if isset($item.description) && $item.description && $settingslb['BON_LOOKBOOK_CUSTOM_PAGE_TEXT'].value}
                                    ...
                                    {if isset($points) && $points && $item.subitems}
                                        ...
                                        {foreach from=$points item=point name=point}
                                            ...
                                            {if $item.id == $point.id_tab && $point.status}
                                                <div class="bonlookbook_point" style="top: {$point.top|escape:'htmlall':'UTF-8' / $item.image_size[1]|escape:'htmlall':'UTF-8' * 100}%; left: {$point.left|escape:'htmlall':'UTF-8' / $item.image_size[0]|escape:'htmlall':'UTF-8' * 100}%">
                                                    ...
                                                    <div class="bonlookbook_point-product {if $point.top >= $item.image_size[1] / 2}bottom-{if $point.left >= $item.image_size[0] / 2}right{else}left{/if}{else}top-{if $point.left >= $item.image_size[0] / 2}right{else}left{/if}{/if}">
                                                        <div class="bonlookboon_point-product-image">
                                                            <a href="{$point.product.link|escape:'htmlall':'UTF-8'}" target="_blank">
                                                                <img src="{$link->getImageLink($point.product.link_rewrite, $point.product_image.id_image, 'home_default')}" alt="{$point.product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}" />
                                                            </a>
                                                        </div>
                                                        <div class="bonlookboon_point-product-info">
                                                            <div class="bonlookboon_point-product-title">
                                                                <a href="{$point.product.link|escape:'htmlall':'UTF-8'}" target="_blank">{$point.product.name|escape:'htmlall':'UTF-8'}</a>
                                                            </div>
                                                            <div class="bonlookboon_point-product-price">
                                                                ...
                                                                {if $point.product.price|string_format:"%.1f" < $point.product.price_without_reduction|string_format:"%.1f"}
                                                                    <p class="after-discount">{$currency.sign|escape:'htmlall':'UTF-8'}{$point.product.price|string_format:"%.1f"|escape:'htmlall':'UTF-8'}</p>
                                                                    <p class="before-discount">{$currency.sign|escape:'htmlall':'UTF-8'}{$point.product.price_without_reduction|string_format:"%.1f"|escape:'htmlall':'UTF-8'}</p>
                                                                {else}
                                                                    <p class="without-discount">{$currency.sign}{$point.product.price|string_format:"%.1f"|escape:'htmlall':'UTF-8'}</p>
                                                                {/if}
                                                            </div>
                                                            <div class="bonlookboon_point-product-link">
                                                                <a href="{$point.product.link|escape:'htmlall':'UTF-8'}" target="_blank">{l s='View Product' mod='bonlookbook'}</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
            ...
            {if $slider_settings['BON_LOOKBOOK_SLIDER_NAV'].value && $slider_settings['BON_LOOKBOOK_PAGE_SLIDER_DISPLAY_CAROUSEL'].value}
