...
class ClassBonlookbook extends ObjectModel
{
    public $id_tab;
    public $id_shop;
    public $title;
    ...
    public function delete()
    {
        ...
        if ($image) {
            if ($image && file_exists(_PS_MODULE_DIR_ . 'bonlookbook/views/img/' . $image)) {
                $res &= @unlink(_PS_MODULE_DIR_ . 'bonlookbook/views/img/' . $image);
            }
        }
    ...
    public static function getBonlookbookList()
    {
        $sql = 'SELECT bonl.*, bonlb.`title`
                FROM ' . _DB_PREFIX_ . 'bonlookbook bonl
                JOIN ' . _DB_PREFIX_ . 'bonlookbook_lang bonlb
