...
class BonCommentCriterion
{
	...
	public static function add($id_lang, $name)
	{
		...
		INSERT INTO `'._DB_PREFIX_.'bon_comment_criterion`
		(`id_lang`, `name`) VALUES(
		'.(int)($id_lang).',
		\''.pSQL($name).'\')'));
	}
	...
	public static function addToProduct($id_product_comment_criterion, $id_product)
	{
		if (!Validate::isUnsignedId($id_product_comment_criterion) ||
			...
			die(Tools::displayError());
		return (Db::getInstance()->execute('
		INSERT INTO `'._DB_PREFIX_.'bon_comment_criterion_product`
	...
	public static function update($id_product_comment_criterion, $id_lang, $name)
	{
		if (!Validate::isUnsignedId($id_product_comment_criterion) ||
			...
			die(Tools::displayError());
		return (Db::getInstance()->execute('
		UPDATE `'._DB_PREFIX_.'bon_comment_criterion` SET
		`name` = \''.pSQL($name).'\'
	...
	public static function getByProduct($id_product, $id_lang)
	{
		...
		INNER JOIN `'._DB_PREFIX_.'bon_comment_criterion_product` pccp ON pcc.`id_product_comment_criterion` = pccp.`id_product_comment_criterion`
	...
	public static function delete($id_product_comment_criterion)
	{
		...
		if ($result === false)
			return ($result);
		return (Db::getInstance()->execute('
		DELETE FROM `'._DB_PREFIX_.'bon_comment_criterion`
		WHERE `id_product_comment_criterion` = '.(int)($id_product_comment_criterion)));
	}
};