...
$(window).on('resize', function () {
	$('#mobile_top_menu_wrapper').css('top', $("#header").height()).css('height', 'calc(100vh - '+$('#header').height()+'px');
...
function openMobileMenu() {
	if ($(window).width() < 992) {
		$("#mobile_top_menu_wrapper").append($('.header-contact.left-block'));

	} else {
		$('.header-contact-info').prepend($('.header-contact.left-block'))
	}
	setTimeout(() => {
		$('#mobile_top_menu_wrapper').css('top', $("#header").height()).css('height', 'calc(100vh - '+$('#header').height()+'px');
	}, 400);
